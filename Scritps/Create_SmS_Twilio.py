...
def save_Cuenta(side, Token, NumATA):
    ...
    carpeta = 'Scritps/cache/Acconts_sms/'
    archivo = nombre + ' - (Twilio).json'
    ...
    current_directory = carpeta = 'Scritps/cache/Acconts_sms/'
    file_path = os.path.join(current_directory, 'SMS_Cuentas.txt')
    with open(file_path, 'w') as file:
        file.write("Text-Cuentas-Cache")
    print(f"\nArchivo de Registro y cache 'SMS_Cuentas.txt' creado ¡exitosamente!")
    
    reverse(seg=3)
    subprocess.run(['python', 'Tools/SmsManager.py'])


def Update_flash_Twilio():
    try:
        subprocess.check_call([sys.executable, '-m', 'pip', 'show', 'twilio'])
        print(f"La Libreria Twilio ya está instalada.")
    except subprocess.CalledProcessError:
        print(f"La Libreria Twilio no está instalada.")
        reverse(seg=1)
        print("Se Iniciara La Instalacion...")
        subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'twilio'])
        clear()
        print("La Instalacion se Completo...")
        Barra(desc="Comprobando la Instalacion...")
        Update_flash_Twilio()
...
print("Verificando La Existencia De La Libreria Twilio...")
reverse(seg=2)
Update_flash_Twilio()
reverse(seg=2)
clear()
